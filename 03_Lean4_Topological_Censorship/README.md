...
**Status at v5.6.0 (2026-09-18):** ten verified files, **85 declarations** checked with `#print axioms`, no `sorry`, no axioms beyond Lean's standard `propext`, `Classical.choice`, `Quot.sound`. Each file's header has a "what is proved / what is not proved" section; read it before citing a theorem.
...
| [`QuantumVortexLink.lean`](src/QuantumVortexLink.lean) | 11 | The quantum-fluid counterpart (paper §9.5): a singly quantized vortex has `u r/(ħ/m) = 1` at every radius; sonic radius `ħ/(mc) = √2 ξ`; Landau's criterion; Bogoliubov critical velocity equals `c` (lower bound and infimum); free-gas critical velocity 0; the mass-independent identity `ν_m/v_u = (√2/4π) a_B` and the resulting floor on `ℓ*`. | Anything about solutions of the Gross–Pitaevskii equation; the two literature bounds entering the floor are estimates, not theorems. |
