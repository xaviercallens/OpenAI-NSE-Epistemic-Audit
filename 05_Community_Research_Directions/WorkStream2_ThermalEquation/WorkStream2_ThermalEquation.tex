...
\paragraph{Update (17 September 2026, v5.5.0): the estimate measured.} A compressible Navier--Stokes--Fourier simulation of a forced collapsing core (\texttt{experiments/compressible\_core.py}; paper \S9.3) gives the actual temperature rise in a gas: the coefficient in $\Delta T = C\,u^2/c_p$ is $C\approx0.35$ at $\mathrm{Ma}\approx0.4$, rising to $C\approx1$ near $\mathrm{Ma}\approx1$, so $u^2/c_p$ is an upper-order estimate, as the caveat anticipated. Two further findings: heat acts on the collapse only through the temperature dependence of viscosity; and on a Reynolds-number-one core (the route of the OpenAI construction) it does not stop the collapse before $\ell_*$, whereas on a high-Reynolds-number (inertial) route air does not follow a driven swirl past a local Mach number of about $0.70$ --- a lock on the Mach number, not on velocity or core size, in a one-dimensional ideal-gas model.
